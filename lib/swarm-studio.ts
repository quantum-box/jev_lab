...
export function stepSwarm(run:SwarmRun):SwarmRun{
 ...
 // Reserve movement destinations before mutating robot positions. Keeping the
 // step-start occupancy set intentionally conservative prevents both entering
 // an occupied cell and two robots choosing the same cell in one step.
 ...
 for(const robot of robots){
  ...
  if(destination!==current)reserved.add(destination); robot.x=nextX; robot.y=nextY; robot.battery--; events.push({step,type:'move',robotId:robot.id,jobId:job.id,detail:`(${robot.x},${robot.y})`});
