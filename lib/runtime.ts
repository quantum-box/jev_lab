...
export type RuntimeTraceEvent<S, A> =
  ...
  | { type: 'pause' | 'resume' | 'reset' | 'cap' | 'complete' | 'error'; step: number; reason?: string };
...
export class ContinuousRuntime<S, A> {
  ...
  private completeCheck() { if (this.config.isComplete?.(this.stateValue)) { this.statusValue = 'completed'; this.traceValue.push({ type: 'complete', step: this.stepValue, reason: 'terminal state reached' }); return true; } return false; }
  async tick(deltaMs: number): Promise<void> { if (!Number.isFinite(deltaMs) || deltaMs < 0) throw new RuntimeError('invalid_delta', 'deltaMs must be non-negative'); this.elapsedValue += deltaMs; if (this.capCheck() || this.statusValue !== 'running') return; this.stateValue = this.config.updateEnvironment(this.stateValue, this.random, this.elapsedValue); if (this.completeCheck()) return; const cadence = this.config.decisionCadenceMs ?? 1000; if (this.elapsedValue - this.lastDecisionAt >= cadence) await this.step(); if (this.statusValue === 'running') this.completeCheck(); this.capCheck(); }
