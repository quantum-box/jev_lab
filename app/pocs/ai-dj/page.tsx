...
export default function AiDj() {
  ...
  useEffect(() => { volumeRef.current = volume; if (masterRef.current) masterRef.current.gain.value = muted ? 0 : Math.min(MAX_VOLUME, volume); }, [volume, muted]);
  ...
  function scheduleNext() {
    ...
    window.setTimeout(() => {
      if (requestToken !== tokenRef.current || requestGeneration !== generationRef.current || statusRef.current !== 'playing') return;
      const next = selectDjLoop(scenario.seed, promptRef.current, currentBar);
      decisionRef.current = next; decisionsRef.current = [...decisionsRef.current, next].slice(-MAX_DECISIONS); traceRef.current = { schema: 'jev-ai-dj-trace', version: 1, seed: scenario.seed, events: [...traceRef.current.events, { type: 'prompt', bar: currentBar, prompt: promptRef.current }, { type: 'decision', bar: currentBar, decision: next }, { type: 'bar', bar: currentBar, loopId: next.loop.id }] };
  ...
  async function startPlayback() {
    ...
    barRef.current = 0; setBar(0); decisionsRef.current = []; setDecisions([]); traceRef.current = { schema: 'jev-ai-dj-trace', version: 1, seed: scenario.seed, events: [] }; setTrace(traceRef.current);
    const AudioContextCtor = window.AudioContext ?? (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
    if (AudioContextCtor) {
      try { const context = audioRef.current ?? new AudioContextCtor(); audioRef.current = context; masterRef.current = masterRef.current ?? context.createGain(); masterRef.current.gain.value = muted ? 0 : Math.min(MAX_VOLUME, volumeRef.current); if (!masterConnectedRef.current) { masterRef.current.connect(context.destination); masterConnectedRef.current = true; } await context.resume(); setAudioMode('web-audio'); } catch { setAudioMode('visual-fallback'); }
