...
export async function POST(request: Request) {
  ...
  const b = body as Record<string, unknown>, actionIds = b.actionIds, tactic = b.tactic;
  ...
  if (tactic !== undefined && (typeof tactic !== 'string' || tactic.length > 2_000)) return NextResponse.json({ error: 'tactic が不正です。', code: 'invalid_tactic' }, { status: 400 });
  ...
  try {
    const response = await fetch(`${process.env.TACHYON_API_URL ?? 'https://api.n1.tachy.one'}/v1/ai/judgments`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'x-operator-id': tenant, 'content-type': 'application/json' }, signal: controller.signal, body: JSON.stringify(buildRuntimeJevRequest(b.snapshot as object, actionIds as string[], tactic)) });
