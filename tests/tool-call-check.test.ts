...
test('prototype names are denied as unknown tools without throwing', () => { assert.doesNotThrow(() => checkToolCall({ tool: 'constructor', args: {} })); assert.equal(checkToolCall({ tool: 'constructor', args: {} }).status, 'deny'); assert.equal(checkToolCall({ tool: '__proto__', args: {} }).status, 'deny'); });
