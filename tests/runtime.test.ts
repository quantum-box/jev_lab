...
test('terminal states complete and prevent later ticks', async () => { const r = new ContinuousRuntime<{ done: boolean }, 'finish'>({ initialState: { done: false }, seed: 1, allowedActions: ['finish'], updateEnvironment: state => state, applyAction: () => ({ done: true }), validateAction: () => ({ ok: true }), isComplete: state => state.done, safeAction: 'finish', decide: async () => ({ action: 'finish' }) }); await r.step(); assert.equal(r.status, 'completed'); await r.tick(100); assert.equal(r.stepCount, 1); });
